...
namespace desperate_housewife 
{
      ...
      for(unsigned int i=0; i < joint_handles_.size(); i++) 
      {
        ...
        if (start_flag)
        {
          ...
          for(unsigned int j=0; j< list_of_link.size(); j++)
          {
            ...
            // ForceAndIndex_table = RepulsiveWithTable(-Table_position.z() + x_chain[list_of_link[j]].p.z());   
  ...
  // std::pair<Eigen::Matrix<double,6,1>, double> PotentialFieldControl::RepulsiveWithTable(double distance_local_obj)
  // {
