...
void phobic_mp::GetJacobian(int &p)
{
    ...
    for(int i=0; i<7;i++)
    {
      ...
      // //UPDATE KINEMATICS
      // Vito_desperate.link_jac_solver_[i]->JntToJac(Vito_desperate.link_joints_[i], Vito_desperate.link_jac_[i]); //jac
      // // update frames
      // Vito_desperate.link_fk_solver_[i]->JntToCart(Vito_desperate.link_joints_[i], Vito_desperate.link_frame_[i]); //x
      // update jacobian with chage of frame
     // Vito_desperate.link_jac_[i].changeRefFrame(Vito_desperate.link_frame_[i]); //is not kinect frame!!
    }
...
void phobic_mp::SetPotentialField_robot(std::vector<Eigen::VectorXd> &Force_repulsion, int &p)
{
    ...
    if(p ==0)
    {
      ...
      { 
        ...
        // *robot_link_position1 = Vito_desperate.robot_position_left;
        // *robot_link_position2 = Vito_desperate.robot_position_right;
      }

        for (int i=0; i< Vito_desperate.Pos_HAND_l_x.size(); i++ )
        {
          HAND =  Vito_desperate.Pos_HAND_l_x[i]; 
        }

        base_link = Vito_desperate.left_arm_base_link_st;
     ...
     else //right arm
     {
       ...
       { 
          ...
          GetEuleroAngle(Vito_desperate.link_frame_l[i], *robot_link_position2[i]);   
             // *robot_link_position1 = Vito_desperate.robot_position_right;
             // *robot_link_position2 = Vito_desperate.robot_position_right;
        }
        
        for (int i=0; i< Vito_desperate.Pos_HAND_r_x.size(); i++ )
        {
          HAND =  Vito_desperate.Pos_HAND_r_x[i];
        }

        base_link = Vito_desperate.right_arm_base_link_st;
     } 
    ...
    {
      if(distance_link[i] < = influence) //if in minus than influence area
      {
        Eigen::Vector3d distance_der_partial;
        distance_der_partial[0] =  distance_link[i](0) / sqrt(pow(distance_link[i](0))+ pow(distance_link[i](1)) +pow(distance_link[i](2)));
        distance_der_partial[1] =  distance_link[i](1) / sqrt(pow(distance_link[i](0))+ pow(distance_link[i](1)) +pow(distance_link[i](2)));
        distance_der_partial[2] =  distance_link[i](2) / sqrt(pow(distance_link[i](0))+ pow(distance_link[i](1)) +pow(distance_link[i](2)));
       ...
       // vec_Temp[0] = (P_obj/pow(distance_link[i],2)) * (1/distance_link[i] - 1/influence) *distance_der_partial[0];
       // vec_Temp[1] = (P_obj/pow(distance_link[i],2)) * (1/distance_link[i] - 1/influence) *distance_der_partial[1];
       // vec_Temp[2] = (P_obj/pow(distance_link[i],2)) * (1/distance_link[i] - 1/influence) *distance_der_partial[2];
