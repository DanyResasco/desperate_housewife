...
namespace desperate_housewife 
{
  ...
  bool PotentialFieldControl::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
  {
      ...
      // pub_error_int_ = nh_.advertise<std_msgs::Float64MultiArray>("error_interpolate", 1000);
       pub_error_int_ = nh_.advertise<desperate_housewife::Error_msg>("error_interpolate", 1000);
  ...
  {   
      ...
      // KDL::Twist x_error_to_send;
        tf::twistKDLToMsg (x_err_,  error_pose.error_);
        ...
        if (start_flag)
        {
          ...
          tau_.data = (J_.data.transpose()*lambda_*(Force_attractive ))+ Force_total_rep + 0.0*N_trans_*(Eigen::Matrix<double,7,1>::Identity(7,1)*(phi_ - phi_last_)/(period.toSec()));
          ...
          KDL::Twist temp;
          temp = diff(x_, x_des_int);
          tf::twistKDLToMsg (temp,  error_pose.error_);
           // error_pose.id = error_pose.id; 
          // error_pose.error_ = diff(x_, x_des_int);
          // x_error_to_send = x_err_.id;
      }
      ...
      // {
      //   err_msg = x_error_to_send(i));
      //   // std::cout<<"x_error_to_send(i): "<<x_error_to_send(i)<<std::endl;
      // }
       
      pub_error_int_.publish(error_pose);
