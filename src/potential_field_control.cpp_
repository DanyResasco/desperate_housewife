...
bool PotentialFieldControl::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
{
  ...
  if (!ros::param::search(n.getNamespace(),"robot_description", robot_description)){
    ROS_ERROR_STREAM("PotentialFieldControl: No robot description (URDF) found on parameter server ("<<n.getNamespace()<<"/robot_description)");
    return false;
    }
  if (!nh_.getParam("root_name", root_name)){
    ROS_ERROR_STREAM("PotentialFieldControl: No root name found on parameter server ("<<n.getNamespace()<<"/root_name)");
    return false;
    }
  if (!nh_.getParam("tip_name", tip_name)){
    ROS_ERROR_STREAM("PotentialFieldControl: No tip name found on parameter server ("<<n.getNamespace()<<"/tip_name)");
    return false;
    }
  ...
  sub_gains_ = nh_.subscribe("gains", 1, &PotentialFieldControl::setGains, this);
  sub_posture_ = nh_.subscribe("command_configuration", 1, &PotentialFieldControl::commandConfiguration, this);
...
void PotentialFieldControl::commandConfiguration(const std_msgs::Float64MultiArray::ConstPtr &msg){
  if (msg->data.size() == 0) {
...
void PotentialFieldControl::setGains(const std_msgs::Float64MultiArray::ConstPtr &msg){
