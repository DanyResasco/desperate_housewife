...
namespace desperate_housewife 
{
  ...
  bool PotentialFieldControl::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
  {
    ...
    for (unsigned int i = 0; i < parameters_.pf_list_of_links.size(); ++i)
    {
      ...
      else
      {
        ...
        ROS_DEBUG("Chain from %s to %s correctly initialized", parameters_.root_name.c_str(), parameters_.pf_list_of_links[i].c_str());
      }
    }

    ROS_DEBUG("Subscribed for desired_reference to: %s", "command");
    sub_command = n.subscribe("command", 1, &PotentialFieldControl::command, this); 
          //list of obstacles
    ROS_DEBUG("Subscribed for obstacle_avoidance_topic to : %s", topic_obstacle_avoidance.c_str());
    sub_obstacles = n.subscribe(topic_obstacle_avoidance.c_str(), 1, &PotentialFieldControl::InfoGeometry, this);

    sub_start_controller = n.subscribe("start_controller", 1, &PotentialFieldControl::startControllerCallBack, this);
    ...
    pub_total_wrench = nh_.advertise<geometry_msgs::WrenchStamped>("wrench_repulsive_table", 512);
  ...
  {   
    ...
    if (start_controller)
    {
      ...
      if (parameters_.enable_obstacle_avoidance)
      {
        ...
        F_repulsive  = getAdjointT( x_.Inverse() ) * (F_table_base);
      }
      ...
      total_repulsive_wrench_end_efector.wrench.force.x = F_total(0);
      total_repulsive_wrench_end_efector.wrench.force.y = F_total(1);
      total_repulsive_wrench_end_efector.wrench.force.z = F_total(2);
      total_repulsive_wrench_end_efector.wrench.torque.x = F_total(3);
      total_repulsive_wrench_end_efector.wrench.torque.y = F_total(4);
      total_repulsive_wrench_end_efector.wrench.torque.z = F_total(5);
  ...
  Eigen::Matrix<double,6,1> PotentialFieldControl::GetFIRAS(double &min_distance, Eigen::Vector3d &distance_der_partial , double &influence)
  {
    ...
    Force(0) = (parameters_.pf_repulsive_gain/pow(min_distance,2)) * distance_der_partial[0];
    Force(1) = (parameters_.pf_repulsive_gain/pow(min_distance,2)) * distance_der_partial[1];
    Force(2) = (parameters_.pf_repulsive_gain/pow(min_distance,2)) * distance_der_partial[2];
    Force(3) = 0;
    Force(4) = 0;
    Force(5) = 0;
  ...
  {
    n.param<std::string>("topic_obstacle", topic_obstacle_avoidance, "obstacles");
    n.param<std::string>("tip_name", parameters_.tip_name, "end_effector");
    n.param<std::string>("root_name", parameters_.root_name, "world");
    n.param<double>("time_interpolation", parameters_.max_time_interpolation, 1);
    n.param<double>("max_tua_percentage", parameters_.max_tau_percentage,0.5);
    n.param<double>("pf_repulsive_gain", parameters_.pf_repulsive_gain ,1);
    n.param<double>("pf_dist_to_obstacles", parameters_.pf_dist_to_obstacles ,1);
    n.param<double>("pf_dist_to_table", parameters_.pf_dist_to_table ,1);

    n.param<bool>("enable_obstacle_avoidance", parameters_.enable_obstacle_avoidance ,true);
    n.param<bool>("enable_joint_limits_avoidance", parameters_.enable_joint_limits_avoidance ,true);
