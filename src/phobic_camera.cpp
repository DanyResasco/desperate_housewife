...
void phobic_scene::pointcloudCallback(sensor_msgs::PointCloud2 msg)
{
  ...
  if (cloud->points.empty ())
  ...
  else
    {
      ...
      for(unsigned int i=0; i<cyl_list.size();i++)
        {
          ...
          ROS_DEBUG("info radius %f", cyl_list[i].radius);
  ...
  while ( object_cluster.size() > 0 )
    {
      ...
      cloud_first->header.frame_id = camera_frame.c_str();
      ...
      if (inliers_cylinder->indices.size() == 0)
      ...
      else
        {
          ...
          std::string cyl= cylinders_topic.c_str() + std::string("_") + std::to_string(a);
          tf_br.sendTransform(tf::StampedTransform(cyl_list[a].cyl_tf_pose, ros::Time::now(), camera_frame.c_str(), cyl.c_str()));
...
void phobic_scene::makeInfoCyl(std::vector<float> coeff , pcl::PointCloud<pcl::PointXYZRGBA>::Ptr pc_cyl, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_scene)
{
  ...
  // bool ok_transf;
  // ok_transf = 
  fromEigenToPose( T_k_g , CYLINDER.Cyl_pose );
...
std::vector<double> findHeight(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_t)
{
  ...
  for (unsigned int i=1; i< cloud_t->points.size(); i++)
  {
...
int phobic_scene::FullorEmpty(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud22)
{
  ...
  ROS_DEBUG("k_indices %lu", k_indices.size());
...
{
  ROS_DEBUG("send a marker msg");

  visualization_msgs::Marker marker;
  marker.header.frame_id = camera_frame.c_str();
...
void phobic_scene::getcluster()
{
  ...
  ROS_DEBUG("PointCloud in getcluster() has: %lu data points.", cloud_filtered->points.size ());
