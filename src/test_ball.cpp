...
namespace desperate_housewife 
{
  ...
  bool ball::init(ros::NodeHandle &nh)
  {
      //ball   
  ...
  void ball::starting(const ros::Time& time)
  {
  ...
  void ball::update(const ros::Time& time, const ros::Duration& period)
  {   
      F_repulsive = GetFieldRep(ball_pos);
      KDL::JntArray Kp,Kd;
      
      x_err_ = diff(frame_init,frame_des);
      for(int i = 0; i < Force_attractive.size(); i++)
      {
          Kp(i) = 200;
          Kd(i) = 100;
          Force_attractive(i) =  (-Kd(i)*(x_dot(i)) + Kp(i)*x_err_(i))/mass;
      }

      // KDL::Frame Force = FromEigenToKdl(F_repulsive+Force_attractive);
      Eigen::Matrix<double,6,1> Force_tot = Force_attractive + F_repulsive;
      GetVelocityAndPosition(Force_tot, period.toSec()); // Force is just divide by mass
  ...
  void ball::GetVelocityAndPosition(Eigen::Matrix<double,6,1> &Force,  double delta)
  {
    ...
    Eigen::Matrix<double,6,1> pos_last = pos;
   
    x_dot = velocity_last + delta * Force;
    pos = 0.5 * Force * delta * delta + x_dot * delta + pos_last;
  ...
  {
      ball_pos.data[0] = msg->data[0];
      ball_pos.data[1] = msg->data[1];
      ball_pos.data[2] = msg->data[2];
      radius = msg->data[3];
      mass = msg->data[4];
      
      pos_des.data[0] = msg->data[5];
      pos_des.data[1] = msg->data[6];
      pos_des.data[2] = msg->data[7];

      SeeMarker(pos_des, "pose_desired", frame_des);
      SeeMarker(ball_pos, "pose_init",frame_init);
    
  }

  void ball::SeeMarker(KDL::Vector &Pos, std::string obst_name, KDL::Frame &frames)
  {
...
// PLUGINLIB_EXPORT_CLASS(desperate_housewife::ball, controller_interface::ControllerBase)